...
name and will wait until it get RS Server request for info after getting connected to client,
TLDS 1 COM server will look for the information that RS server didn't find in its database. 
IF TLDS 1 COM server has information it will return info with a flag A.
else return HOSTname Error.

TLDS 1 COM Server will take input of txt file during the Run Command, 
For Example: python3 ./TSCOM.py PROJ2-DNSCOM.txt 
...
def server():
    try:
        ...
        print("[S]: TLDS1 COM Server socket created")
    ...
    print ("[S]: Got a connection request from a RS server at", addr)
...
# accepting data and look into disctionary if it exists, if not send ERROR msg. 
    while (True):
        ...
        if data_from_client in TSDNS_tabel:
            ...
            seradd="HOSTNAME ERROR: HOST NOT FOUND in TSCOM Server"
